...
class GameHomePage extends Component {
  ...
  componentDidMount() {
    this.timerId = setInterval(this.startRemainingTimer, 1000)
  }

  startRemainingTimer = () => {
    const {timer, isWon} = this.state
    if (timer > 1 && isWon === true) {
      this.setState(prevState => ({
        timer: prevState.timer - 1,
      }))
    } else {
      clearInterval(this.timerId)
      this.setState({timer: 0, isWon: false})
    }
  }
  ...
  getRandomMatchedImage = () => {
    const {activeImage} = this.state

    return (
      <div className="matched-image-container">
        <img className="matched-image" alt="match" src={activeImage.imageUrl} />
      </div>
    )
  }
  ...
  onClickTabItem = id => {
    const {activeImage, score} = this.state

    if (id === activeImage.id && score < 30) {
      this.setState(prevState => ({
        score: prevState.score + 1,
        activeImage: [...imagesList].sort(() => Math.random() - 0.5)[0],
      }))
    } else {
      this.setState({isWon: false})
    }
  ...
  render() {
    const {activeTabId, score, timer, isWon} = this.state
    ...
    return (
      <div className="main-container">
        ...
        <div className="bottom">
          {isWon === false ? (
            <ResultPage score={score} restartGame={this.restartGame} />
          ) : (
            <>
              {this.getRandomMatchedImage()}
              <ul className="tab">
                {tabsList.map(eachTab => (
                  <li key={eachTab.tabId}>
                    <button
                      type="button"
                      onClick={() => this.setActiveTabId(eachTab.tabId)}
                      className={`button ${
                        activeTabId === eachTab.tabId ? 'styling' : ''
                      }`}
                    >
                      {eachTab.displayText}
                    </button>
              ...
              <ul className="list">
                {activeTabImages.map(eachItem => (
                  <li key={eachItem.id} className="list-item">
                    <button
                      onClick={() => this.onClickTabItem(eachItem.id)}
